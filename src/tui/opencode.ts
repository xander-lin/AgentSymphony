...
interface OpenCodeTuiClient {
  session?: {
    promptAsync(input: { path: { id: string }; body: { parts: Array<{ type: "text"; text: string }>; variant?: string }; query?: { directory?: string } }): Promise<unknown>
  }
}
...
export class OpenCodeTuiController implements TuiController {
  ...
  async injectPrompt(text: string, options: { variant?: string } = {}): Promise<void> {
    ...
    await this.client.session.promptAsync({
      ...
      body: { parts: [{ type: "text", text }], variant: options.variant },
    })
  }
}
